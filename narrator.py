...
def generate_scripts_from_prompt(prompt_text, num_videos, api_key=None, logger=None):
    """
    Calls the Gemini API to generate `num_videos` scripts based on the provided Markdown prompt.
    Returns a list of clean string scripts.
    """
    if not api_key:
        api_key = os.getenv("GEMINI_API_KEY")
    if not api_key:
         if logger: logger("Error: GEMINI_API_KEY not found in environment or arguments. Please add it to your .env file.")
    ...
    try:
        ...
        full_prompt = (
            f"Please generate {num_videos} distinct short video scripts based exactly on the following criteria/prompt.\n"
            "CRITICAL: The output MUST be a JSON array with EXACTLY " + str(num_videos) + " elements.\n"
            "Each element in the array must be ONE JSON OBJECT with exactly two keys: 'script' and 'caption'.\n"
            "Inside 'script', provide the complete script string. You MUST separate each sentence or short phrase with a newline character (\\n). Keep each phrase under 7 words.\n"
            "Inside 'caption', provide a catchy social media caption containing emojis and relevant hashtags.\n"
            "Respond ONLY with the valid JSON array of objects.\n\n"
            f"--- PROMPT BEGIN ---\n{prompt_text}\n--- PROMPT END ---\n"
        )
        ...
        if logger: logger("Calling Gemini API to generate scripts...")
        response = model.generate_content(full_prompt, generation_config=generation_config)
        
        scripts = json.loads(response.text)
        if isinstance(scripts, list):
            valid_scripts = [s for s in scripts if isinstance(s, dict) and 'script' in s and str(s['script']).strip()]
            if logger: logger(f"Successfully generated {len(valid_scripts)} scripts via Gemini.")
            return valid_scripts
        else:
            if logger: logger("Gemini API returned JSON, but it was not an array of strings.")
            return []
    except Exception as e:
...
def process_narrator_batch(scripts, gameplay_dir, output_dir, use_openai=False, api_key=None, speed_factor=1.0, logger=None):
    ...
    for i, script_obj in enumerate(scripts):
        if isinstance(script_obj, dict):
            ...
            caption_text = script_obj.get('caption', script_text)
        else:
        ...
        try:
            ...
            for line_idx, line in enumerate(lines):
                ...
                # Trim silence (tightened padding for faster pacing)
                audio_clip = trim_silence(audio_clip, threshold=0.01, padding=0.05)
            ...
            temp_audio_path = os.path.join(output_dir, f"temp_narrator_audio_{i}.m4a")
            
            if logger: logger(f"  Exporting: {out_name}")
            
            # Remove existing files to prevent ffmpeg overwrite hanging or conflicts
            if os.path.exists(out_path):
                try:
                    os.remove(out_path)
                except Exception as ex:
                    if logger: logger(f"  Warning: Could not remove existing file {out_name}: {ex}")
